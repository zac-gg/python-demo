...
cfg = ConfigParser()
cfg.read(ini_file)
const.REFUND_QUERY_PARAMETER = '/nh-gateway/card/refundQuery'

class RefundQueryOrder():
	def refund_query(self):
		api_url = cfg['config']['REQUEST_URL']+const.REFUND_QUERY_PARAMETER
		#Get timestamp
	...
	def __get_sign_data(self,now):
		...
		return dict
t = RefundQueryOrder()
status_code,text=t.refund_query()
