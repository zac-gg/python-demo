...
cfg = ConfigParser()
cfg.read(ini_file)
const.REFUND_CANCEL_PARAMETER = '/nh-gateway/card/refundCancel'

class RefundCancelOrder():
	def refundCancel(self):
		api_url = cfg['config']['REQUEST_URL']+const.REFUND_CANCEL_PARAMETER
		#Get timestamp
	...
	def __get_post_data(self,sign,now):
		post_dict = {
			...
			'timeStamp': now,
			'language' : 'en',
			'refundOrderNo': 'XP01OC3210720190104827779475298',
		}
	...
	def __get_sign_data(self,now):
		...
		return dict
t = RefundCancelOrder()
status_code,text=t.refundCancel()
